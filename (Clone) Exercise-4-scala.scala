...
val dataDF: DataFrame = spark.createDataFrame(
  dataRDD.map(row => Row(row.getDouble(0), row.getDouble(1))),
  StructType(Seq(
...
val Array(trainingDF, testDF) = dataDF.randomSplit(Array(0.8, 0.2), seed = 1)
...
val testPredictions: DataFrame = lrModel.transform(
  vectorAssembler.transform(testDF)
  )
...
val values = Seq(-2.8, 3.14, 9.9, 123.45)
val inputDF = values.toDF("X")

val predictions = lrModel.transform(
  vectorAssembler.transform(inputDF)
  )
...
val bestDaysDF: DataFrame = salesDF
  ...
  .groupBy("orderDate")
  .agg(sum("saleTotal").as("totalSales"))
...
val myStreamingIdentifier: String = "mikko_kukkonen"
